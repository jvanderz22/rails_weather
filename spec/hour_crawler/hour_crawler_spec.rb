describe HourCrawler do
  ...
  let(:forecast) { hour_crawler.forecast }
  describe "#forecast" do
    ...
    it "should correctly set the date" do
      expect(forecast[0][:date]).to eq("07/10")
      expect(forecast[1][:date]).to eq("07/10")
      expect(forecast[10][:date]).to eq("07/11")
      expect(forecast[23][:date]).to eq("07/11")
    end
